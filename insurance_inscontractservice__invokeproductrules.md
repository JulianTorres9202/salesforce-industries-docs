---
title: "InsContractService:invokeProductRules"
source: "https://help.salesforce.com/s/articleView?id=ind.insurance_inscontractservice__invokeproductrules.htm&language=en_US&type=5"
scraped: "2026-04-14"
filter: "ind.insurance_"
---# InsContractService:invokeProductRules

InsContractService:invokeProductRules[](https://help.salesforce.com/s?language=en_US)
...
For example, if you'd added rules that send certain contracts to an underwriter for approval, this service invokes those rules.
...
1.  Finds the contract object based on the `objectID` and extracts the products for that object based on that `objectID`. The service queries the contract object's line items.
...
The contract object ID being queried.
...
The state the service transitions the contract object to when all rules run by the service evaluate to false.
...
Describes what the service changed in the Status field on the contract object.
...
Tells whether or not the service transitions the contract object to a new state.
